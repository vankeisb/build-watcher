...
fetch build time =
    ...
    , (case build.def of
        ...
        JenkinsDef i data ->
            Jenkins.fetch data
                |> Task.map (\br -> ( br, JenkinsDef i data ))
      )
...
updateAddBuildView abvm model =
    case abvm of
                                            ...
                                            (List.indexedMap
                                                (\i pb ->
                                                    case pb of
                                                        ...
                                                        PersistedJenkinsBuild tags d ->
                                                            JenkinsDef { id = i + newCounter, tags = tags } d
                                                )
                                            )
                                        |> Result.withDefault []
        ...
        ABJenkinsJobChanged s ->
            noCmd <|
                mapJenkinsBuildData
                    model
                    (\( t, te ) ->
                        ( { t | job = s }
                        , { te | job = validateRequired s }
                        )
                    )
        ...
        BVCopyClicked build ->
            let
                ...
                newDef =
                    case build.def of
                        BambooDef cd d ->
                            BambooDef (newCd cd) (Bamboo.copy d)

                        TravisDef cd d ->
                            TravisDef (newCd cd) (Travis.copy d)

                        JenkinsDef cd d ->
                            JenkinsDef (newCd cd) (Jenkins.copy d)
...
mapJenkinsBuildData : Model -> (( Jenkins.Data, Jenkins.ValidationErrors ) -> ( Jenkins.Data, Jenkins.ValidationErrors )) -> Model
mapJenkinsBuildData model f =
    let
        abd =
            model.addBuildData

        ( data, errors ) =
            f ( abd.jenkins, abd.jenkinsErrors )
    in
        { model
            | addBuildData =
                { abd
                    | jenkins = data
                    , jenkinsErrors = errors
                }
        }
