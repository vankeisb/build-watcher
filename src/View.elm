            ...
            [ div
                ...
                [ Options.styled p
                    ...
                    [ text <|
                        ...
                        else if String.isEmpty model.filterText then
    ...
    let
        ( rows, canSave ) =
            case model.addBuildData.tab of
                ...
                2 ->
                    ( jenkinsRows model
                    , Jenkins.canSave model.addBuildData.jenkins
                    )
...
jenkinsRows : Model -> List (Grid.Cell Msg)
jenkinsRows model =
    let
        jenkins =
            model.addBuildData.jenkins

        jenkinsErrors =
            model.addBuildData.jenkinsErrors
    in
        [ formRow <|
            withHelp "URL (and port if any) of the Jenkins server" <|
                Textfield.render Mdl
                    [ 6, 0 ]
                    model.mdl
                    (tfOpts
                        [ Textfield.label "Server URL"
                        , Textfield.value jenkins.serverUrl
                        , Options.onInput <| onInputAbv ABJenkinsServerUrlChanged
                        , textfieldError jenkinsErrors.serverUrl
                        ]
                    )
                    []
        , formRow <|
            withHelp "Job Name" <|
                Textfield.render Mdl
                    ...
                    (tfOpts
                        ...
                        , Textfield.value jenkins.job
                        , Options.onInput <| onInputAbv ABJenkinsJobChanged
                        , textfieldError jenkinsErrors.job
                        ]
                    )
                    []
        ]
    ...
    Dialog.view
        ...
        , Dialog.content []
            [ p []
                [ text <|
                    "version : "
                        ++ model.flags.appVersion
                ]
            , p []
                [ text <|
                    model.flags.appName
                        ++ " monitors builds from several C.I. servers."
                ]
            , p []
